...
A @t2t assembly is essential in gaining complete insight into an organism's genome. A @t2t sequence represents a complete, continuous genome without gaps or fragmentation @t2t-genome-assembly. Remarkably, achieving such assemblies has only become feasible in recent years with the advent of long-read sequencing technologies. Although the Human Genome Project concluded in 2003 @hgp-facts, the reference genome produced contained several missing regions that were challenging to assemble, with the first @t2t sequencing of human DNA only recently achieved in 2021 @t2t-chm13.
...
#place(top + center)[#subpar.grid(
  ...
  caption: [The figures show common artifacts in overlap graphs generated from real read data, taken from human chromosome 19 (a, b) and 21 (c). Note that these are the very same overlap graphs that are successfully simplified and resolved by our work. Graphs visualized via @graphia.],
  label: <full>,
)]
...
#v(-0.5em)
#figure(image("graphics/mamba_official.png", height: 3.5cm), caption: [Mamba's selection mechanism in #text(fill:blue.darken(50%))[blue] alters parameters in an input-dependent manner. Diagram from @mamba.]) <fig:mamba_official>
...
This success extends to the test set where all three models see a significant uplift to the inverse precision and inverse F1 score, with negligible impact on other metrics such as accuracy. For example, with @symgatedgcn, ultra-long read data helps to improve inverse precision from $41.2%$ to $52.3%$, and the inverse F1 score from $52.5%$ to $60.4%$ (@tab:symgatedgcn_test). The complete suite of data can be found @app:chr-19-ul-granola-test, which shows analogous performance improvements for GAT+Edge and SymGAT+Edge. These findings provide compelling evidence that all tested @gnn:pl are successfully able to leverage ultra-long data to improve erroneous edge detection in overlap graphs.
...
Furthermore, the Graph Transformer architecture @graph-transformer can also be trialed on this problem. By not leveraging the graph connectivity inductive bias, performance may be improved especially across long distances in the graph---potentially particularly useful with ultra-long read data. This could also help surface and mitigate any over-squashing @over-squashing and under-reaching @under-reaching phenomenon encountered by any of the tested architectures. Additionally, while we currently focus our efforts on detecting and removing erroneous edges, Graph Transformer Networks could establish new connections between unconnected nodes on the original graph @graph-transformer-new-connections, helping improve assembly contiguity. Unfortunately, due to compute limitations, we were unable to experiment with this architecture for ourselves.
...
The tables below contain the experimental results of running the GAT+Edge and SymGAT+Edge architectures on overlap graphs with ultra-long data, and with @granola additionally added.
...
caption:[There is a significant performance uplift when ultra-long data is integrated into the overlap graph (GAT+Edge), as was observed with @symgatedgcn, with much higher inverse precision and F1 score #highlighted. @granola does not help in improving performance further. The results show the mean and standard deviation across 5 runs, with chromosome 15 used for training, and 9 as the testing dataset for these metrics. Best results in *bold*. $arrow.t$ indicates _higher is better_.]
) <tab:gat_test>
...
caption: [Integration of ultra-long data into the overlap graph (GAT+Edge (UL)) results in a higher fraction of the reference genome being covered in the reconstructed assembly #highlighted, as was observed with @symgatedgcn. This is achieved whilst maintain assembly quality. @granola is detrimental to assembly quality. The results show the mean and standard deviation across 5 runs, with chromosome 15 used for training, and 9 as the reference genome assembled. Best results in *bold*. $arrow.t$ indicates _higher is better_. $arrow.b$ indicates _lower is better_.]
) <tab:gat_assembly>
