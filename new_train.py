...
def train(train_path, valid_path, out, assembler, overfit=False, dropout=None, seed=None, resume=False, finetune=False, ft_model=None):
    ...
    (loaded_graph,), _ = dgl.load_graphs(train_path)
    ...
    model = models.SymGatedGCNModel(node_features, edge_features, hidden_features, hidden_edge_features, num_gnn_layers, hidden_edge_scores, batch_norm, dropout=dropout)
    model.to(device)
                ...
                for data in ds_train:
                    ...
                    for sub_g in sub_gs:
                        ...
                        if use_symmetry_loss:
                            ...
                            ol_len = g.edata["overlap_length"].to(device)[sub_g.edata[dgl.EID]].float()
                            ol_len = (ol_len - ol_len.mean()) / ol_len.std()
                            ol_sim = g.edata['overlap_similarity'].to(device)[sub_g.edata[dgl.EID]]
                            e = torch.cat((ol_len.unsqueeze(-1), ol_sim.unsqueeze(-1)), dim=1).to(device)
                            pe_in = g.in_degrees().float().to(device)[sub_g.ndata['_ID']].unsqueeze(1).to(device)
                            pe_in = (pe_in - pe_in.mean()) / pe_in.std()
                            pe_out = g.out_degrees().float().to(device)[sub_g.ndata['_ID']].unsqueeze(1).to(device)
                        ...
                        if num_nodes_for_g >= g.num_nodes(): # full graph
                            ...
                            for sub_g in sub_gs:
                                if use_symmetry_loss:
                                    ...
                                    # x = g.ndata['x'][sub_g.ndata['_ID']].to(device)
                                    # e = g.edata['e'][sub_g.edata['_ID']].to(device)
                                    ol_len = g.edata["overlap_length"][sub_g.edata[dgl.EID]].float()
                                    ol_len = (ol_len - ol_len.mean()) / ol_len.std()
                                    e = ol_len.unsqueeze(-1).to(device)
                                    pe_in = g.ndata['in_deg'][sub_g.ndata['_ID']].unsqueeze(1).to(device)
                                    pe_in = (pe_in - pe_in.mean()) / pe_in.std()
                                    pe_out = g.ndata['out_deg'][sub_g.ndata['_ID']].unsqueeze(1).to(device)
                                    pe_out = (pe_out - pe_out.mean()) / pe_out.std()
...
if __name__ == '__main__':
    # python3 new_train.py --train chm13-data/chr19/ --valid chm13-data/chr19/ --overfit --asm hifiasm > hq-training
