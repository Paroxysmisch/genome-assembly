...
d_state = 16
batch, length, dim = 2_000, 8_000, 4 
...
# chunk_size = 10
# y = torch.zeros(batch, dim)
# for i in range(batch // chunk_size):
#     y[i * chunk_size : (i + 1) * chunk_size] = model(x[i * chunk_size : (i + 1) * chunk_size])[:, -1, :]
y = model(x)
